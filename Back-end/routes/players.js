...
router.post('/', async (req, res) => {
    try{
        const { fields, values, params } = getFieldsAndValues(req.body);
...
router.put('/:id', async (req, res) => {
    try{
        ...
        const { fields, values } = getFieldsAndValues(req.body);

        const setClause = fields.map((field, index) => `${field} = $${index + 1}`).join(', ');

        const response = await pool.query(`UPDATE players SET ${setClause} WHERE id = $${values.length + 1}`,[...values, id]);
